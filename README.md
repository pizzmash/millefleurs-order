...
| 人気順と注文数       | 家ごとの累計杯数でメニューを表示。家主は「注文」→「注文数」で確認・一括リセット |
...
- 注文には受付時のレシピを保存します。
- 人気順はその家で受付が成立した杯数の多い順です。同数は名前順になります。既存注文はDB移行時に集計します。
- 家主の「注文数」タブで全カクテルの累計を確認・検索できます。リセットは全杯数を0に戻し、注文履歴・受付中の注文を残します。
