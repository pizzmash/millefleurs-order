...
const fail = (status: ApiError['status'], message: string): never => {
  throw new ApiError(status, message);
};
const integer = (value: unknown) => {
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0)
    fail(400, '材料またはカクテルの指定が正しくありません。');
  return value as number;
};
const parseBody = async (request: Request): Promise<Record<string, unknown>> => {
  if (!request.headers.get('content-type')?.includes('application/json'))
    fail(400, 'JSON形式で送信してください。');
  try {
    const body = await request.json();
    if (!body || Array.isArray(body) || typeof body !== 'object')
      fail(400, '入力を確認してください。');
    return body;
  } catch {
...
const pageNumber = (value: string | undefined) => {
  ...
  if (!Number.isSafeInteger(page) || page < 1 || page > 1000000)
    return fail(400, 'ページ番号を確認してください。');
  return page;
};
export function createApp(db: DatabaseSync, options: { publicUrl?: string } = {}) {
  ...
  app.use('/api/*', async (c, next) => {
    ...
    c.set(
      'guest',
      token
        ? (db.prepare('SELECT id,nickname FROM guests WHERE id=?').get(token) as
  ...
  const findOrder = (id: string) =>
    db.prepare('SELECT * FROM orders WHERE id=?').get(id) as DbOrder | undefined;
  ...
  app.post('/api/join', async (c) => {
    ...
    if (typeof nickname !== 'string' || !nickname.trim() || nickname.trim().length > 24)
    ...
    db.prepare(
      'INSERT INTO guests VALUES(?,?) ON CONFLICT(id) DO UPDATE SET nickname=excluded.nickname',
    ).run(guest.id, guest.nickname);
    setCookie(c, 'bar_guest', guest.id, {
      httpOnly: true,
      sameSite: 'Lax',
      path: '/',
      maxAge: 60 * 60 * 24 * 365,
    ...
    for (const key of ['min', 'max', 'kind', 'page']) {
      ...
      if (
        ...
        (!Number.isFinite(Number(value)) ||
          Number(value) < 0 ||
          (['kind', 'page'].includes(key) && !Number.isInteger(Number(value))))
      )
        return fail(400, '絞り込み条件を確認してください。');
    }
    if (query.get('min') && query.get('max') && Number(query.get('min')) > Number(query.get('max')))
  ...
  app.put('/api/host/inventory/:id', async (c) => {
    ...
    if (!db.prepare('SELECT id FROM drinks WHERE id=?').get(drinkId))
      return fail(404, '材料が見つかりません。');
    db.prepare(
      'INSERT INTO inventory VALUES(?,?) ON CONFLICT(drink_id) DO UPDATE SET available=excluded.available',
    ).run(drinkId, +available);
  ...
  app.get('/api/orders', (c) => {
    ...
    const rows = db
      .prepare(
        'SELECT * FROM orders WHERE guest_id=? ORDER BY created_at DESC,id DESC LIMIT ? OFFSET ?',
      )
      .all(guest.id, limit + 1, (page - 1) * limit) as DbOrder[];
  ...
  app.post('/api/orders', async (c) => {
    ...
    if (typeof input.requestKey !== 'string' || !/^[\w-]{16,100}$/.test(input.requestKey))
      return fail(400, '注文の送信情報を確認してください。');
    const requestKey = input.requestKey;
    const result = transaction(db, () => {
      const previous = db
        .prepare('SELECT * FROM orders WHERE guest_id=? AND request_key=?')
        .get(guest.id, requestKey) as DbOrder | undefined;
      if (previous) {
        if (previous.cocktail_id !== cocktailId)
          fail(409, '別の注文としてもう一度操作してください。');
        return previous;
      }
      const cocktail = getCatalog(db).find((row) => row.id === cocktailId);
      if (!cocktail) return fail(404, 'カクテルが見つかりません。');
      if (!cocktail.available)
        return fail(409, '材料の在庫が変わったため、現在このカクテルは注文できません。');
  ...
  app.get('/api/host/orders', (c) => {
    ...
    const rows = db
      .prepare(
        `SELECT * FROM orders WHERE status=? ORDER BY created_at ${status === 'pending' ? 'ASC' : 'DESC'},id LIMIT ? OFFSET ?`,
      )
      .all(status, limit + 1, (page - 1) * limit) as DbOrder[];
    const pendingCount = (
    ...
    return c.json({
      orders: rows.slice(0, limit).map(present),
  ...
  app.put('/api/host/orders/:id/ingredients/:recipeId', async (c) => {
    ...
    const updated = transaction(db, () => {
      ...
      const drinks = getDrinks(db);
      const currentKind =
        drinks.find((d) => d.id === ingredient.drinkId)?.kindId ?? ingredient.kindId;
      const selected = candidates(ingredient.drinkId, currentKind, drinks).find(
        (d) => d.id === selectedId,
      );
      if (!selected) return fail(409, '在庫が変わりました。現在使える材料から選び直してください。');
      ingredient.selectedId = selected.id;
      ingredient.selectedName = selected.name;
      db.prepare('UPDATE orders SET ingredients=? WHERE id=?').run(
        JSON.stringify(ingredients),
        order.id,
      );
  ...
  app.post('/api/host/orders/:id/complete', async (c) => {
    ...
    const updated = transaction(db, () => {
      ...
      if ((JSON.parse(order.ingredients) as OrderIngredient[]).some((i) => i.selectedId === null))
