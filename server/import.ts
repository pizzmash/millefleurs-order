...
export function importCatalog(db: DatabaseSync, directory = 'data/raw') {
  ...
  const data = Object.fromEntries(
    tables.map((name) => {
      const rows = parse(readFileSync(join(directory, `${name}.csv`)), {
        columns: true,
        bom: true,
  ...
  const kinds = refs(data.drink_kind),
    drinks = refs(data.drink),
    cocktails = refs(data.cocktail),
    glasses = refs(data.glass),
    techniques = refs(data.technique);
  for (const d of data.drink)
    if (d.kind_id.trim() && !kinds.has(d.kind_id)) throw new Error(`Unknown kind: drink ${d.id}`);
  for (const c of data.cocktail)
    if (
      (c.glass_id && !glasses.has(c.glass_id)) ||
      (c.technique_id && !techniques.has(c.technique_id))
    )
      throw new Error(`Unknown glass/technique: cocktail ${c.id}`);
  for (const r of data.cocktail_drink)
    if (!drinks.has(r.drink_id) || !cocktails.has(r.cocktail_id))
      throw new Error(`Broken recipe ${r.id}`);
  const withRecipe = new Set(data.cocktail_drink.map((r) => r.cocktail_id));
  const warnings = data.cocktail
    .filter((c) => !withRecipe.has(c.id))
    .map((c) => `No recipe: ${c.id} ${c.name}`);
  transaction(db, () => {
    const k = db.prepare(
      'INSERT INTO kinds VALUES(?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name, alcoholic=excluded.alcoholic',
    );
    ...
    for (const [source, target] of [
      ...
      const st = db.prepare(
        `INSERT INTO ${target} VALUES(?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name`,
      );
      for (const r of data[source]) st.run(id(r.id), r.name);
    }
    const d = db.prepare(
      'INSERT INTO drinks VALUES(?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,kind_id=excluded.kind_id',
    );
    ...
    for (const r of data.cocktail_drink)
      recipe.run(id(r.id), id(r.cocktail_id), id(r.drink_id), r.quantity);
