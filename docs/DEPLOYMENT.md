...
メニュー・カクテル詳細・在庫・招待設定の定期取得は行いません。画面表示、検索・ページ変更、タブ復帰・ウィンドウへのフォーカス復帰で取得します。在庫・設定操作後にも更新します。他端末の変更は次回取得時に反映されます。注文確定時の最新在庫・受付状態・招待検証は維持し、409時には詳細を再取得します。注文完了画面では詳細の取得を停止します。
